...
void loglikGamma_allcomb1(double *logLik, int *nJointCombs, int *NOC, int *NOK, 
	...
	for(int locind=0; locind< *nMarkers; locind++) {	//for each marker:
		...
		for(kk=0; kk< *NOC; kk++) { //for each contributors:
			...
			if(knownGind0>=0) { //If contributor is known (genotype given)
				...
				kindUnknown[jj] = kk; //insert contributor index (IMPORTANT TO INDICATE WHICH CONTRIBUTOR THE UNKNOWN HAS)
