...
void loglikGamma_cumprob(double *pvalVEC, double *maxY, int *nJointCombs, int *NOC, int *NOK, 
	...
	double *AT, double *fst, double *dropinProb, double *dropinWeight1, double *dropinWeight2, //note the additional drop-inWeight2 
	...
	for(int locind=0; locind< *nMarkers; locind++) {	//for each marker:
		...
		for (rr = 0; rr < nReps0; rr++) { //traverse each replicate		
			for (aa = 0; aa < nAlleles; aa++) { //traverse each observed alleles (indices), also the Q-allele. Potential not necessary!
				aaind = aa*nReps0 + rr; //get index vectorized vector which includes (allele,rep) indices as [(1,1), (1,2), (1,3), (2,1), (2,2) etc]
				if( peaksLong[SI_nAllelesReps0 + aaind] < AT[SI_nRepMarkers0 + rr] ) { //no PH observation to consider
				...
				for (int iter = 0; iter < nJointCombs[locind]; iter++) { //for each combined/joint genotype outcome
					...
					vector<int> jointGind(NOU, 0); //This will be the perPHexpation contribution index for the unknown inds (directly corresponds to indices of Gmarg)
					...
					for(r = 0; r < nReps0; r++) { //traverse each replicates (observed alleles indicated by PH)
						...
						for (a = 0; a < nAlleles; a++) { //traverse each observed alleles (also consider the last allele since Q-allele may have PH last allee)				
							...
							//CHECK IF THE LOOPED INDEX IS SAME AS LOOPED ALLELE:
							if( aind!=aaind ) { //proceed as before if different index							
								if( peak >= AT0 ){ //if PH>0 (this is same as PH>=AT since threshold has already been applied)
									if(shapev2[aind] > smalltol) { //If contribution and PH>0  					//CONTRIBUTION SET (A)
										...
										logevidProb += dropinWeight1[ SI_nAllelesReps0 + aind ]; //likelihood for observed dropin (both PH and lambda are included)							
								...
								if(shapev2[aind]>smalltol) { //If contribution and PH>0  					//CONTRIBUTION SET (A)
									...
									logevidProb += log( pgamma(peak, shapev2[aind],scale0[repID], 1, 0) - pgamma(AT0-1, shapev2[aind],scale0[repID], 1, 0));
									//log( gamma_p(shapev2[aind],peak*const1[repID]) -  gamma_p(shapev2[aind], AT0*const1[repID]) ); 																					
								} else { //If contribution and PH>0 //DROPIN SET (C)
									logevidProb += dropinWeight2[ SI_nAllelesReps0 + aind ]; //likelihood for observed dropin (both PH and lambda are included)							
									nDropin[r] += 1; //count dropin for particular replicate											
				   ...
				   //Calculate inner sum:
				   bigsum += val;
				   //bigsumVEC[SI_nJointCombs0 + iter] = val; //insert element
				} //end for each combination iterations (bigsum)
				
				pvalVEC[SI_nAllelesReps0 + aaind] = bigsum; //insert p-value value		
			} //end for each allele
